...
def plot_from_semicolon_data_tk(lines, scale_secondary=1, title="SleepMonitor Plot"):
    ...
    for row in reader:
        ...
        accel.append((float(row["AccelMax"]) / 5000.0) * scale_secondary)
        position.append(10.0 * float(row["Position"]))
        bpm.append(float(row["BreathPerMinute"]))
        bmax.append(float(row["BreathMax"]))
        cSnores.append(float(row["cSnores"]) * scale_secondary)
        csmall.append((float(row["cSmallMoves"]) / 10.0) * scale_secondary)
    ...
    right_series = [
        ...
        ("cSnores (scaled)", cSnores),
        ("cSmallMoves (scaled)", csmall),
    ]

    # Fixed colors (simple)
    colors_left = ["#1f77b4", "#2ca02c", "#d62728"]
