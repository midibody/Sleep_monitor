...
--------------------------------------------------------------------------------------------------------------------------
10 Feb 2026     1.0.1       Corrects bug when launched from c: . DataImport.csv now created in current directory     
14 Feb 2026     1.0.2       Improve graph display, add error handling when failing to create dataImport.csv file 
--------------------------------------------------------------------------------------------------------------------------
> To build: 
    pip install pyserial, etc...

> To generate .exe standalone file: pyinstaller --onefile --name SleepMonitor sleepMonitor.py
  to build .exe, Need to install pyinstaller before: python -m pip install --upgrade pyinstaller
"""
...
#----------------------
""" PREVIOUS GRAPH VERSION NICER BUT: embarks the big Plot library that results into an exe fle of 35 Mbytes
...
#    with serial.Serial(port, BAUDRATE, timeout=READ_TIMEOUT) as ser:
    print (f"Sleep Monitor version {VERSION}\n\n* * *\nWARNING: there is currently a BIG LIMITATION IN DEBUGGING USAGE:\nIf data is recieved asynchronously from a command, the code remains stuck! It happens if you activate debug or plotting functions. \n * * *\nUSB Ports available:")
    #while (cPorts ==0):
    list_arduino_port()
    ...
    while (1):
        ...
        if cmd and (cmd[0] == 'c' or cmd[0] == 'p'): #save data to dataImport.csv file and display graph
            ...
            try:
                ...
                print(f"File {filename} could not be created. Open error: {e}")
            else:
